...
## Never let the store compress with brotli (the 26-second query)

Every store query rides `makeStoreFetch` (`src/lib/store-fetch.ts`), which pins
`accept-encoding: gzip`. That header is load-bearing: Node's fetch otherwise advertises
brotli, the funnel in front of sqld honours it, and the **Zenbook** pays the compression —
which is quadratic in how it feels. Interleaved A/B/A/B against the primary, median of n=5:

| query | default (`br`) | `gzip` | `identity` |
| --- | --- | --- | --- |
| history payload's plays scan (16,375 rows) | **26,436ms** | 648ms | 470ms |
| library payload's member scan (7,087 rows) | **8,479ms** | 367ms | 188ms |
| one-key `meta` read | 58ms | 26ms | 24ms |

Same SQL, same rows: it is all compression CPU on a laptop. Left alone it took production
down on 2026-08-22 — 26s blew the 15s per-attempt timeout, the retry threw (below), so
`/api/search/history` answered **500** and Home fell back to a library-only index in which
every song and artist read "Never played". Symptom in the wild: `TypeError: Response body
object should not be disturbed or locked` on that route, and `/usage` showing Home opens with
`data-rendered` but no `history-ready`. `gzip` and not `identity` because the body still
crosses the internet to Vercel.

## The store fetch must rebuild its Request per attempt

hrana calls `fetch(request)` with a **Request object** whose body is a one-shot stream, so
re-issuing the same object throws `Response body object should not be disturbed or locked`
(undici's `extractBody`, reached through Next's fetch patch) instead of retrying. That made
the DNS-blip ladder unreachable for every store query — they are all POSTs. `makeStoreFetch`
buffers the body once and builds a fresh Request per attempt; `tests/store-fetch.test.ts`
pins it. Don't "simplify" it back to passing `input` through.

## Simplified vs traditional Han are different strings

The same song is in this store twice under two scripts, because Spotify hands the title out
both ways depending on the release you reached it through — verified 2026-08-22: "Ada Zhuang"
has both `两个人的回忆一个人过` and `兩個人的回憶一個人過` as playlist members, and the title
Rem searched (`再见只是陌生人`) was in his library only as `再見只是陌生人`. So **every name
match folds first**: `hanFold` (`src/lib/han-fold.ts`), used by Home's song/artist search and
by `fuzzyFilter`. It folds traditional → simplified, the direction that is nearly 1:1.

`han-fold.ts` is GENERATED by `scripts/gen-han-fold.py` from ICU's `Hant-Hans` transform
(`uconv`, icu4c) — don't hand-edit it, and note the two traps the generator now guards:
uconv drops some unassigned code points (so records are keyed by codepoint, not line order),
and a few simplified variants are outside the BMP (`㠣` → `𫵷`), so the table must be built by
code point — indexing the paired strings by code unit desynchronised it and mapped `見` to
`蜗`. Both are pinned in `tests/han-fold.test.ts`. Cost: 18.6 KB raw / 13.4 KB gzipped in the
client bundle.

What this does NOT do: merge the two spellings into one song. They remain separate identities
in the store, so their play counts and playlist memberships stay separate — search finds both,
and the rows read as two songs.
